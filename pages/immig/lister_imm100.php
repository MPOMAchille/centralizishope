...
FROM 
    userss u
INNER JOIN 
    formulaire_immigration_session1 f1 ON u.id = f1.user_id";
...
FROM 
    userss u
INNER JOIN 
    formulaire_immigration_session2 f2 ON u.id = f2.user_id";
...
FROM 
    userss u
INNER JOIN 
    formulaire_immigration_session3 f3 ON u.id = f3.user_id";
...
?>
    ...
    <input type="text" id="searchInput2" onkeyup="searchTable(2)" placeholder="Rechercher dans Session 2...">
    <table id="table2">
        <thead>
            <tr>
        ...
        <tbody>
        <?php
        if ($result_session2->num_rows > 0) {
            while($row = $result_session2->fetch_assoc()) {
                echo "<tr>
            ...
            echo "<tr><td colspan='10'>Aucun résultat pour la session 2</td></tr>";
        }
        ?>
        </tbody>
    </table>
