...
$sql = "SELECT ds.*, dd.nom, dd.prenom, dd.compte, dd.statut, dd.help2
        FROM formulaire_immigration_session11 AS ds
...
if ($result && $result->num_rows > 0) {
    ...
    <body class="container my-5">
        <h2 class="mb-4">Informations des entreprises / Agences</h2>
        <?php
        $currentUserId = null;
        while ($row = $result->fetch_assoc()) {
            if ($currentUserId !== $row['user_id']) {
                ...
                ?>
                <h3>Entreprise(s) de : <?php echo $row['nom']; ?> <?php echo $row['prenom']; ?> : <strong style="color: blue;"><?php echo $row['compte']; ?></strong> </h3>
            ...
            <tr>
                ...
                <td class="action-buttons">
                    <form action="" method="post" style="display:inline;">
                        ...
                        <button class="btn btn-secondary" type="button" onclick="voirPlus('<?php echo $row['id']; ?>')">Voir plus</button>
                    </form>
                </td>
            </tr>
            <?php
